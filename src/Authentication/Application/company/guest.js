  ...
  newRequest = () => {
    this.props.auth !== undefined && //window.alert("login")
      addDoc(collection(firestore, "proposals"), {
        ...
        .then(() => {
          this.setState({ newRequest: { name: "" }, tempProposal: false });
        })
        .catch(standardCatch);
  };
  render() {
    ...
    return (
      <div>
        ...
        <div //request proposal
          onClick={() =>
            this.setState({ tempProposal: !this.state.tempProposal })
          }
          id="new proposal"
          onMouseEnter={(e) => this.setState({ hovering: e.target.id })}
          onMouseLeave={() => this.setState({ hovering: "" })}
        ...
        <Proposals
          ...
          //hideProposals={this.state.tempProposal}
        />
      </div>
    );
  }
